...
## Email Handler
if node[:chub_sensu_pipeline].attribute?(:email) and node[:chub_sensu_pipeline][:email].attribute?(:recipient) 
...
## PagerDuty Handler
if node[:chub_sensu_pipeline].attribute?(:pagerduty) and node[:chub_sensu_pipeline][:pagerduty].attribute?(:api_key)
