...
node.normal[:chub_mc_webservice][:webservice] = "platform-webservice"
...
node.normal[:chub_mc_webservice][:webservice_name] = "platform_webservice"
...
node.normal[:chub_mc_webservice][:pem_file_dir] = "/etc/mc_platform"

#node.normal[:chub_mc_webservice][:jar_file_url] = node[:chub_mc_webservice][:platform][:jar_file_url]
#node.normal[:chub_mc_webservice][:jar_file_name] = node[:chub_mc_webservice][:platform][:jar_file_name]
...
directory node[:chub_mc_webservice][:pem_file_dir] do
  action :create
  owner "chub_#{node[:chub_mc_webservice][:webservice_name]}"
  group "chub_#{node[:chub_mc_webservice][:webservice_name]}"
  mode 0777
...
template "#{node[:chub_mc_webservice][:pem_file_dir]}/chef-validator.pem" do
  source "chef-validator.pem.erb"
  owner "root"
  group "root"
  mode 0644
  action :create
...
template "#{node[:chub_mc_webservice][:pem_file_dir]}/mc_platform.pem" do
  source "mc_platform.pem.erb"
  owner "root"
  group "root"
  mode 0644
  action :create
