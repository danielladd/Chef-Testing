#
# Cookbook Name:: chub_sensu_albers
# Recipe:: server
#
...
## Email Handler
if node[:chub_sensu_pipeline].attribute?(:email) and node[:chub_sensu_albers][:email].attribute?(:recipient) 
...
## PagerDuty Handler
if node[:chub_sensu_pipeline].attribute?(:pagerduty) and node[:chub_sensu_albers][:pagerduty].attribute?(:api_key)
