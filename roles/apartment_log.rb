...
default_attributes(
	"chub_log" => {
		"types" => {
			"apartment" => {
				"name" => "apartment",
				"body" => "multiline {\n	pattern => \"^[^|]* | \\s\"\n	what => \"previous\"\n}\ngrok {\n	match => [ \"message\", \"%{LOGLEVEL:loglevel}\\s*\\[(?<logdate>%{YEAR}[/-]%{MONTHNUM}[/-]%{MONTHDAY} %{TIME})\\] %{JAVACLASS}: %{GREEDYDATA:message}\" ]\n}\ndate {\n	match => [ \"logdate\", \"YYYY-MM-dd HH:mm:ss,SSS\" ]\n    locale => \"en\"\n}"
			},
			"apartment_wrapper" => {
				"name" => "apartment_wrapper",
				"body" => "multiline {\n	pattern => \"^[^|]* | \\s\"\n	what => \"previous\"\n}\ngrok {\n	match => [ \"message\", \"((?<loglevel>%{LOGLEVEL}|STATUS)) (\\s*\\|\\s*) (?<logwriter>[a-zA-Z0-9\\s]*) (\\s*\\|\\s*) (?<logdate>%{YEAR}[/-]%{MONTHNUM}[/-]%{MONTHDAY} %{TIME}) (\\s*\\|\\s*) %{GREEDYDATA:message}\" ]\n}\ndate {\n	match => [ \"logdate\", \"YYYY/MM/dd HH:mm:ss\" ]\n    locale => \"en\"\n}"
			},
			"apartment_requests" => {
